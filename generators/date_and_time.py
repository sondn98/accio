from utils.assertions import (
    assert_types, assert_gt, assert_in, validate_str_datetime
)
...
class DateGenerator(BaseGenerator):
    ...
    def validate_params(self, params):
        ...
        if params['max_age'] and params['min_age']:
            assert_gt(params['max_age'], params['min_age'],
                      'Dialect param "max_age" must be greater than "min_age"')
        ...
        if params['from_date'] and params['to_date']:
            assert_gt(params['to_date'], params['from_date'],
                      'Param "to_date" must be someday after "from_date"')
...
class DatetimeGenerator(BaseGenerator):
    ...
    def default_params(self):
        return dict(
            ...
            tz='UTC',
            from_datetime=datetime(1970, 1, 1,
                                   0, 0, 0, 0, pytz.utc).strftime(DEFAULT_DATETIME_FORMAT),
    ...
    def validate_params(self, params):
        ...
        if params['from_datetime'] and params['to_datetime']:
            assert_gt(params['to_datetime'], params['from_datetime'],
